...
export default function ColumnGroupingTable({
  ...
  const [janitorTasks, setJanitorTasks] = useState();
  ...
  useEffect(() => {
    axios
      .get(`https://638265da281f14ffefa75b07.mockapi.io/janitor_tasks`)
      .then((res) => {
  ...
  if (
    ...
    rows = janitorTasks.map((janTask) => {
      ...
      const _user = staffs.find(
        (staff) => staff.id === janTask.janitor_id
      ).name;

      const _disposal = disposals.find(
        (disposal) => disposal.id === janTask.disposal_id
      ).name;

      const _mcp = janTask.mcp;

      const _time = Date(janTask.time).substring(
        0,
        Date(janTask.time).length - 25
      );
