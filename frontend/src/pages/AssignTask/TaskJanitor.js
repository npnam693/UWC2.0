...
function TaskJanitor({ triggerJanitor }) {
  ...
  useEffect(() => {
    const fetchTaskJanitor = async () => {
      const resTaskJanitor = await axios(
        `https://638265da281f14ffefa75b07.mockapi.io/janitor_tasks`
      );
    ...
    const fetchListJanitor = async () => {
      ...
      if (resListJanitor.data) {
        ...
        resListJanitor.data.forEach((item) => {
          if (item.isCollector == false) tempNameJanitor.push(item.name)
        })
  ...
  return (
    triggerJanitor ? <Stack alignItems='center'>
      <Stack width='100%' flexDirection='row' justifyContent='space-around'>
        ...
        <Box >
          <Stack flexDirection='row'>
            <Typography m='20px 0' fontSize='22px' fontWeight={600}>List of Janitor</Typography>
          </Stack>
          <Stack spacing={2} sx={{ width: 300 }}>
            <div>
              <Autocomplete
